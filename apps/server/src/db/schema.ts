...
export const tasks = sqliteTable(
  'tasks',
  {
    ...
    classification: text('classification', { mode: 'json' }).$type<TaskClassification>(),
    classifierVersion: text('classifier_version'),
    classificationSource: text('classification_source').$type<'ollama' | 'deterministic'>(),
    classifierFallbackUsed: integer('classifier_fallback_used', { mode: 'boolean' })
      .notNull()
      .default(false),
    modelTier: text('model_tier').$type<ModelTier>(),
    selectedModel: text('selected_model'),
    selectedReasoning: text('selected_reasoning').$type<ReasoningLevel>(),
    modelFallbackUsed: integer('model_fallback_used', { mode: 'boolean' }).notNull().default(false),
    modelSelectionRationale: text('model_selection_rationale'),
    quotaEstimatePercent: real('quota_estimate_percent'),
    quotaEstimateSource: text('quota_estimate_source').$type<'baseline' | 'historical'>(),
    quotaEstimateSampleCount: integer('quota_estimate_sample_count').notNull().default(0),
...
export const executions = sqliteTable(
  'executions',
  {
    ...
    quotaWaitUntil: text('quota_wait_until'),
    complexity: text('complexity').$type<ComplexityClass>(),
    modelTier: text('model_tier').$type<ModelTier>(),
    selectedModel: text('selected_model'),
    selectedReasoning: text('selected_reasoning').$type<ReasoningLevel>(),
    modelFallbackUsed: integer('model_fallback_used', { mode: 'boolean' }).notNull().default(false),
